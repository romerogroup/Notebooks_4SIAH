...
def parse_args():
    ...
    parser.add_argument('--data-dir', type=str, default='.',
                        help='the path for the CIFAR10 dataset')
...
def main():
    ...
    if os.path.isfile(opt.data_dir+os.sep+'cifar-10-binary.tar.gz'):
        shutil.copy(opt.data_dir+os.sep+'cifar-10-binary.tar.gz',opt.output_dir)
    else:
        raise ValueError(f'{opt.data_dir+os.sep}cifar-10-binary.tar.gz could not be found!')
    ...
    def train(epochs, ctx):
        ...
        train_data = gluon.data.DataLoader(
            gluon.data.vision.CIFAR10(root=opt.output_dir,train=True).transform_first(transform_train),
            batch_size=batch_size, shuffle=True, last_batch='discard', num_workers=num_workers)

        val_data = gluon.data.DataLoader(
            gluon.data.vision.CIFAR10(root=opt.output_dir,train=False).transform_first(transform_test),
